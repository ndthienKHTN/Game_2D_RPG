...
{
    public class Gorgon : MonoBehaviour, IEnemyController
    {
        [Header("Attributes")]
        ...
        private IEnumerator AttackPlayer()
        {
            ...
            if (attackEffect != null)
            {
                ...
                if (playerObj != null)
                {
                    // Hiển thị hiệu ứng tấn công tại vị trí của Player
                    Vector3 attackEffectPosition = playerObj.transform.position;
                    GameObject effectInstance = Instantiate(attackEffect, attackEffectPosition, Quaternion.identity);
                    effectInstance.SetActive(true);

                    // Gây sát thương và làm bất động Player
                    IPlayerController playerController = playerObj.GetComponent<IPlayerController>();
                    if (playerController != null)
                    {
                        // Gây sát thương
                        playerController.beAttacked(null, Attack);

                        // Làm bất động Player
                        playerController.DisableMovement(2f); // Hàm giả định để làm Player bất động
                    }

                    // Hiệu ứng tồn tại trong 2 giây
